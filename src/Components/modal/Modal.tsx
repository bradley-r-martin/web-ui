...
const Modal: FunctionComponent<ModalProps> & ModalComposition = (props: ModalProps) => {
  const { children, onDismiss, size, anchored } = props
  ...
  return (
    <Portlet>
      <Container>
        ...
        <motion.div
          key='modal'
          className={`bg-white relative shadow-2xl  max-h-full overflow-auto  max-w-full w-full md:w-auto z-30 ${size}
          ${anchored === undefined ? 'rounded-t-2xl md:rounded-md' : ''}
          ${anchored === 'top' ? 'rounded-b-2xl' : ''}
          ${anchored === 'bottom' ? 'rounded-t-2xl' : ''}
